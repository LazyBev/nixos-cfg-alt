...
source "/home/yari/nixos-cfg/devenvs/rust/.devenv/shell-env.sh"
...
if [ ! -x "/run/current-system/sw/bin/fish" ] && ! command -v "/run/current-system/sw/bin/fish" >/dev/null 2>&1; then
    echo "devenv: error: shell '/run/current-system/sw/bin/fish' not found" >&2
    echo "devenv: add fish to your devenv.nix packages or set SHELL to an absolute path" >&2
    exit 1
fi
exec "/run/current-system/sw/bin/fish" -i -C "source /home/yari/nixos-cfg/devenvs/rust/.devenv/devenv.fish"
